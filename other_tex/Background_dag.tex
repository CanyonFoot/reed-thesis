...
\theoremstyle{definition}
\begin{definition}
A $\mathbf{graph}$ $G$ is an ordered pair $G = (V, E)$ where $V$ denotes a set of nodes (sometimes called vertices) and $E \subseteq \{(i,j) | i,j \in V\}$ denotes a set of edges between nodes.
\end{definition}
...
\begin{definition}
A $\mathbf{path}$ between nodes $v_1, v_n$ is a sequences of $v_1, v_2, ..., v_n$ with $v_i \in V$ for all $i \in \{1,2,...,n \}$ such that an edge always exists between $v_i$ and $v_{i+1}$. If $v_i \rightarrow v_{i+1} \in E$ for all $i \in \{1,2,...,n \}$, we say $v_i$ is a $\mathbf{directed \ path}$.
\end{definition}
...
\begin{definition}
A directed graph $G = (V,E)$ is said to be an \textbf{directed acyclic graph} (DAG) if for all nodes $v \in V$, $DE_v \cap AN_v = \emptyset$. That is, a directed graph is a DAG when the existence of directed path from $v_1$ to $v_2$ implies that there is not a directed path from $v_2$ to $v_1$.
\end{definition}
...
The graphical structure that every SCM entails allows us to reason about the causal and statistical relationships using the properties of directed graphs. One of the essential such tools is d-separation. To develop the intuition, consider the very basic DAG $A \rightarrow B \rightarrow C$. There is exactly one directed path from $A$ to $C$, and the path passes through $B$. So to get to $C$ from $A$ you need to pass through $B$. In this way, we say that $B$ \emph{blocks} the directed path from $A$ to $C$. We extend this notion [ref Pearl 2008].

\begin{definition}
Given a directed graph $G$, a path $p$ is said to be blocked by a set $S$ (rephrase to note that S cannot contain starting or ending nodes of the path) if:
\begin{enumerate}
\item $j \in S$ and $p$ contains a subpath $i \rightarrow j \rightarrow k$, $i \leftarrow j \leftarrow k$ (called a chain), or $i \leftarrow j \rightarrow k$ (called a fork) or

\item $j \notin S$ and $DE_j \cap S = \emptyset$ and $p$ contains a subpath $i \rightarrow j \leftarrow k$. The structure $i \rightarrow j \leftarrow k$ is called a \textbf{collider} and is particularly important.
\end{enumerate}
\end{definition}

\begin{example}
Consider a path $A \rightarrow B \rightarrow C \leftarrow D \rightarrow E$ between nodes $A$ and $E$. There are many sets that block this path. For one, there is a collider on the path. This means that and set which does not include $C$ or any elements of $DE_C$ would successfully block the path. However, we could also pick $S = \{B\}$, $S= \{D\}$, or $S = \{B,D\}$ as blocking sets since the path contains a 'chain' at $B$ and a  'fork' at $D$. Importantly, if we have either of these nodes in our blocking set, we could include $C$ as well because the first blocking condition would be satisfied, even though the second would not. In fact, the only $S \subseteq \{B,C,D\}$ that would not block the path is $S = \{C\}$.
\end{example}

With blocking defined, we address d-separation. 

\begin{definition}
For a DAG $G$, two sets of nodes $A,B \in G$ are said to be \textbf{d-separated} by a set $S$ if all paths between $A$ and $B$ are blocked by $S$. We denote d-separation by the symbol $\perp \!\!\!\perp_G$, with the statement "$A$ is d-separated from $B$ by $S$" expressed as $A \dsep_G B | S$. 
\end{definition}
...
In any SCM $C$ with entailed graph $G$, if $A \dsep_G B | S \implies A \dsep B | S$ for all disjoint sets of nodes $A,B,S$. That is, every d-separation in the graph corresponds to a conditional independence in the joint distribution. 
\end{definition}
